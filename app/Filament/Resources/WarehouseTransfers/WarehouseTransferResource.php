...
use App\Filament\Resources\WarehouseTransfers\Pages;
...
class WarehouseTransferResource extends Resource
{
        ...
        return $schema
            ->schema([
                Section::make('اطلاعات سند')
                    ->description('اطلاعات پایه انتقال')
                    ...
                    ->schema([
                        Forms\Components\TextInput::make('document_number')
                            ->label('شماره انتقال')
                            ->default(fn () => 'TRF-'.\Morilog\Jalali\Jalalian::forge('today')->format('Ymd').'-'.(now()->hour * 3600 + now()->minute * 60 + now()->second).'-'.rand(1000, 9999))
                            ->disabled()
                            ->dehydrated(),
    ...
    {
        return $table
            ->modifyQueryUsing(fn ($query) => $query->where('document_type', Document::TYPE_TRANSFER))
            ->columns([
